...
interface LabsProjectsExpandableProps {
  projects: LabProject[];
}
...
export default function LabsProjectsExpandable({ projects }: LabsProjectsExpandableProps) {
  ...
  return (
    <section className="px-4 sm:px-8 max-w-7xl mx-auto mb-8 space-y-12 pt-12 pb-12 relative z-10">
          ...
          <div className="fixed inset-0 grid place-items-center z-[100] px-4 py-4">
            ...
            >
              ...
              <div className="flex flex-col h-full overflow-y-auto">
                <motion.div layoutId={`lab-image-${active.title}-${id}`}>
                ...
                <div className="p-5 sm:p-6 flex-grow">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4">
                    <motion.h3
                      layoutId={`lab-title-${active.title}-${id}`}
                      className="font-bold text-lg sm:text-2xl text-base-content pr-2"
                    >
                      {active.title}
                    </motion.h3>
                    <motion.div layoutId={`lab-button-${active.title}-${id}`} className="flex-shrink-0">
                      <Link
                        href={`/portfolio/${active.slug}`}
                        className="btn btn-ghost btn-sm w-full sm:w-auto text-sm opacity-80"
                      >
                        View Details
                      </Link>
                    </motion.div>
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-semibold text-sm sm:text-base text-base-content/80 mb-2">Technologies:</h4>
                      <div className="flex flex-wrap gap-1.5 sm:gap-2">
                        {active.tech.map((t, i) => (
                          <span 
                            key={i} 
                            className="px-2.5 sm:px-3 py-1 bg-base-300/50 text-base-content/70 rounded-full text-xs sm:text-sm font-medium border border-base-300"
                          >
                            {t}
                          </span>
                        ))}
      ...
      {/* Card Grid (Collapsed State) - Smaller cards with reduced opacity */}
